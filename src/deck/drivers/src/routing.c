...
static void processRoutingDataMessage(UWB_Packet_t *packet) {
  ...
  PACKET_LOSS_RATE[neighborAddress] =
      (double) LOSS_COUNT[neighborAddress] / (RECEIVE_COUNT[neighborAddress] + LOSS_COUNT[neighborAddress]);
...
static void printDebugInfo() {
  ...
  DEBUG_PRINT("min: %.2f, max: %.2f, average: %.2f, total loss: %lu, total received: %lu, neighbor count: %d \n",
              minPacketLossRate * 100,
              maxPacketLossRate * 100,
              averagePacketLossRate * 100 / neighborCount,
              lossCount,
              receiveCount,
              neighborCount);
}
